...
import { Clock, Users, Star, Award, BookOpen, Play, Filter, Search, Loader2 } from 'lucide-react'
...
import { courses as courseAPI } from '../utils/supabase/client'
...
export function CoursesSection({ onStartQuiz }: CoursesSectionProps) {
  ...
  const loadUserEnrollments = async () => {
    try {
      ...
      const enrolled = data.enrollments.map((e: any) => e.course_id)
      const progress = data.enrollments.reduce((acc: any, e: any) => {
                  ...
                  {enrolledCourses.includes(course.id) && (
                    <div className="mb-4">
                      <div className="flex justify-between text-sm mb-2">
                        <span>Progress</span>
                        <span>{enrollmentProgress[course.id] || 0}%</span>
                      </div>
                      <Progress value={enrollmentProgress[course.id] || 0} className="h-2" />
                    </div>
                  )}
